...
def download_pdfs(cleaned_urls, folder):
    """
    ...
    for url in cleaned_urls:
        pdf_url = f"https://{url}"
        file_path = search_save_pdf(pdf_url, folder)
        if file_path:
            num_articles += 1
    return num_articles
...
def download_pdf(file_name, folder):
    ...
    pdf_urls = fetch_pdf_urls(base_urls)
    cleaned_urls = clean_pdf_urls(pdf_urls)
    num_articles = download_pdfs(cleaned_urls, folder)
